...
pub struct Settings {
    ...
    pub whitelist_user_agent: Vec<String>,
...
impl Settings {
    pub fn load() -> Result<Self> {
        ...
        settings.search_mode = parse_bool(&env::var("SEARCH_MODE").unwrap_or_else(|_| "false".to_string()));
        ...
        settings.search_prompt = env::var("SEARCH_PROMPT")
        ...
        // List configurations
        settings.blocked_models = parse_comma_separated(&env::var("BLOCKED_MODELS").unwrap_or_default());
        settings.whitelist_models = parse_comma_separated(&env::var("WHITELIST_MODELS").unwrap_or_default());
        settings.whitelist_user_agent = parse_comma_separated(&env::var("WHITELIST_USER_AGENT").unwrap_or_default());
        ...
        if settings.enable_storage {
            if let Ok(persistent_settings) = persistence::load_settings(&settings.storage_dir) {
                settings = merge_settings(settings, persistent_settings);
            }
        }
    ...
    pub fn save(&self) -> Result<()> {
        if self.enable_storage {
            persistence::save_settings(self, &self.storage_dir)?;
        }
        Ok(())
    }
...
fn merge_settings(env_settings: Settings, persistent_settings: Settings) -> Settings {
    // Priority: environment variables override persistent settings
    Settings {
        password: if env::var("PASSWORD").is_ok() { env_settings.password } else { persistent_settings.password },
        web_password: if env::var("WEB_PASSWORD").is_ok() { env_settings.web_password } else { persistent_settings.web_password },
        gemini_api_keys: if env::var("GEMINI_API_KEYS").is_ok() { env_settings.gemini_api_keys } else { persistent_settings.gemini_api_keys },

        fake_streaming: if env::var("FAKE_STREAMING").is_ok() { env_settings.fake_streaming } else { persistent_settings.fake_streaming },
        concurrent_requests: if env::var("CONCURRENT_REQUESTS").is_ok() { env_settings.concurrent_requests } else { persistent_settings.concurrent_requests },

        // For most settings, prefer environment over persistent
        ..env_settings
    }
}